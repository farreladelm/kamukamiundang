    ...
    <section>
      ...
      <div className="mt-8 overflow-x-auto border border-stone-300 bg-white">
        <table className="w-full min-w-[42rem] text-left text-sm">
          ...
          <tbody>{orders.map((order) => <tr key={order.id} className="border-b border-stone-200 last:border-0"><td className="p-4">{order.customer.name}</td><td className="p-4">{order.templateKey} v{order.templateVersion}</td><td className="p-4">{order.status}</td><td className="p-4">{order.invitation ? <Link href={`/admin/invitations/${order.invitation.id}`} className="underline">Invitation</Link> : "-"}</td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
