...
export const rsvpSubmissionSchema = z.object({
  ...
  if (value.attendance === "ATTENDING" && (value.guestCount < 1 || value.eventKeys.length === 0)) {
    context.addIssue({ code: "custom", path: ["eventKeys"], message: "Pilih acara dan jumlah tamu." });
  }
  if (value.attendance !== "ATTENDING" && (value.guestCount !== 0 || value.eventKeys.length > 0)) {
    context.addIssue({ code: "custom", path: ["eventKeys"], message: "Pilihan acara tidak sesuai status kehadiran." });
  }
});
